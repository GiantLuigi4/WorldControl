...
public class WorldControl {
	...
	public static int structX, structY, structZ;
	public static String structName = "";
	public static StructureThing struct = new StructureThing(new BlockContext[0]);
	...
	public WorldControl() {
		...
		MinecraftForge.EVENT_BUS.addListener(this::chats);
	...
	private void chats(ServerChatEvent event) {
		if (event.getMessage().equals("!ToggleSlowMode" ) && event.getPlayer().hasPermissionLevel(4)) {
			...
			return;
		} else if (event.getMessage().equals("!SMHelp" )) {
			event.setCanceled(event.isCancelable());
			return;
